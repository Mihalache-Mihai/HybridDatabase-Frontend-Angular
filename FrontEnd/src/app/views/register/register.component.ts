...
} from '../../shared/service/authentication.service';
import {
  Router
} from '@angular/router';
import {
...
export class RegisterComponent implements OnInit {
  ...
  onSubmit() {
    ...
    // this.registerService.register(username,password,firstName,lastName,email,cnp,salary).(user=>{
    //   if(user!=null){
    ...
    user = this.registerService.register(username, password, firstName, lastName, email, cnp, salary).then(user => {
      if (user != null) {
        this.registerService.login(username, password).subscribe(empl => {
          if (empl != null) {
            this.router.navigate(['/dashboard']);
